...
export async function update(dbname, transcation) {
  try {
    const userDB = nano.use(dbname);
    console.log(`Updating account record : ${JSON.stringify(account)}`);
    const res = await userDB.insert(account);
    console.log(`Updating account response : ${JSON.stringify(res)}`);
  } catch (e) {
    console.log(`Exception while updating account ${e}`);
  }
}
